	...
	public class Button : ElementUI
	{
		...
		{ 
			...
			MouseEntered += (_,_) => 
			{
				...
				EventsUI.AddEvent(new(0, 15, name+"-onhover", EventQueueBehavior.Discard,
					(ge) => { HoverProgress = Easing.SmoothStep(ge.Progress); }));
			};
			MouseLeft += (_, _) =>
			{
				EventsUI.RemoveEvent(name + "-onhover");
				EventsUI.AddEvent(new(0, 15, name + "-onhoverend", EventQueueBehavior.Discard,
